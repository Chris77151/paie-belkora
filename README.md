...
| **Couleur de marque par société** (`src/lib/brand-color.ts`, + 10 tests) : chaque société définit une `brand_color` (choisie ou **extraite du logo**, canvas) dans Paramètres → un **spectre mono-teinte harmonieux** (accent/en-tête/tint/encre) est dérivé et appliqué à ses bulletins (PDF + HTML). Sans couleur : vert Miya par défaut, à l'identique. | ✅ + 10 tests |
