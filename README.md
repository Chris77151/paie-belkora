...
| **Journal des connexions** (`src/lib/login-audit.ts`, moteur pur + tests) — carte **réservée au super administrateur** dans Paramètres. Trace **chaque tentative d'authentification** (réussie **et** échouée) : date/heure, identifiant saisi, compte/rôle/société si connu, résultat et **cause de l'échec** (champs vides / identifiant inconnu / compte désactivé / mot de passe incorrect). **Sécurité** : le mot de passe n'est **jamais** journalisé, même en échec ; journal **borné** aux 500 plus récentes (anti-croissance), filtre réussite/échec, purge. Capté dans `login()`, persisté via le store (partagé Supabase si activé). | ✅ + 6 tests |
